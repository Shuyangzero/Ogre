...
import numpy as np
import random
from ibslib.structures.structure import StoicDict, Structure
from ibslib.structures import structure_handling
...
def main(list_of_structures, replica, nmpc=None):
    '''
...
#        self.ui = user_input.get_config()
    ...
    def get_mol_list(self, geo):
        ''' Returns input geometry grouped into arrays of molecules 
            ...
            This function will never work for multiple molecule systems.
        '''
    ...
    def combine_orientation_info(self, orientation_info_a, orientation_info_b): 
        ...
        # Randomly permute indices of molecules paired for mating with respect
        #  to the other 
        ...
        # Iterate through j while iterating through i
        j = range(n)
        for num,i in enumerate(perm[p_i]):
            child_z, child_y, child_x  = ( 
                np.array(orientation_info_a[j[num]][:3])*rand_cut + 
                np.array(orientation_info_b[i][:3])*(1-rand_cut))
            if parent_a_COM: 
                COM = orientation_info_a[j[num]][3]
            else:
                COM = orientation_info_b[i][3]
            if parent_a_mol:
                centered_mol = orientation_info_a[j[num]][4]
            else:
    ...
    def random_orientation(self, struct):
        '''
        10% of the time chooses to rotate the system before performing 
          crossover
        '''
        rand = random.random()
        ...
        if rand < 0.1:
            ...
            if choice == "cab":
                ...
                structure_handling.move_molecule_in(struct, self.num_mols,
    ...
    def output(self, message): print(message) #output.local_message(message, self.replica)
