...
from ibslib.io import check_dir,check_struct_dir
...
def check_aims(struct_path):
    """
    1. Will check the directory containing the struct path if it's a valid FHI-aims
       calculation directory. 
    2. If it is, it checks for an FHI-aims aims.out file. 
    3. If it exists, checks if the calculation is completed. 
    4. If the calculation is complete. 
       
       True:  Extract and write results to the Structure.json file in 
              struct_path. This includes either the SCF energy or writing the 
              relaxed geometry if a relaxation was performed. This file will 
              then be moved to the output directory automatically by the Check 
              class. 
       
       False: Check the calculation if an error has occured. If it has,  
              change geometry.in.next_step to geometry if a relaxation is being
              performed. Returning False will then cause resubmission of the 
              calculation.
...
class Check(_NaiveParallel):
    ...
    def calculate(self):
        """
        Simple calculate function using ibslib style calculators. In addition,
        ...
        if self.calculator != None:
            ...
            for calc_dir in self.my_files:
                ...
                if len(s) == 1:
                    ...
                    self.calculator.write(self.output_dir, overwrite=True)
                else:
            ...
            if self.my_filename_dict.get(file_name) != None:
                ...
                if self.check_fn(file_path, **self.check_fn_kw):
                    ...
                    if move:
                        ...
                        write(self.output_dir, temp_dict)
                        shutil.rmtree(calc_dir)
